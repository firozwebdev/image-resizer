...
export const VALIDATION_RULES = {
  ...
  MAX_FILES: 2000, // Increased to 2000
  MIN_DIMENSION: 1,
  MAX_DIMENSION: 16384, // Increased to 16K
  SUPPORTED_FORMATS: [
    "image/jpeg",
    "image/png",
    "image/webp",
...
export function validateFiles(files) {
  ...
  if (files.length > VALIDATION_RULES.MAX_FILES) {
    errors.push(
      `Too many files: ${files.length} (max: ${VALIDATION_RULES.MAX_FILES})`
    );
  }
...
export function validateSettings(settings) {
  ...
  if (
    settings.width &&
    (settings.width < VALIDATION_RULES.MIN_DIMENSION ||
      settings.width > VALIDATION_RULES.MAX_DIMENSION)
  ) {
    errors.push(
      `Width must be between ${VALIDATION_RULES.MIN_DIMENSION} and ${VALIDATION_RULES.MAX_DIMENSION} pixels`
    );
  }

  if (
    settings.height &&
    (settings.height < VALIDATION_RULES.MIN_DIMENSION ||
      settings.height > VALIDATION_RULES.MAX_DIMENSION)
  ) {
    errors.push(
      `Height must be between ${VALIDATION_RULES.MIN_DIMENSION} and ${VALIDATION_RULES.MAX_DIMENSION} pixels`
    );
  }
  ...
  if (
    settings.format &&
    !VALIDATION_RULES.SUPPORTED_FORMATS.includes(settings.format)
  ) {
...
export function validateImageMetadata(metadata) {
  ...
  if (
    metadata.width > VALIDATION_RULES.MAX_DIMENSION ||
    metadata.height > VALIDATION_RULES.MAX_DIMENSION
  ) {
    errors.push(
      `Image dimensions too large: ${metadata.width}×${metadata.height} (max: ${VALIDATION_RULES.MAX_DIMENSION}×${VALIDATION_RULES.MAX_DIMENSION})`
    );
  }
...
export function checkBrowserCompatibility() {
  ...
  return {
    ...
    features,
    unsupported,
    warnings: features.webp
      ? []
      : ["WebP format not supported in this browser"],
  };
}
